...
<body>
  ...
  <!-- ./services_system_test/services-index.php -->
  <!-- ./public/services.php -->
  <!-- ./public/services.php -->
  ...
  <!--<body>-->
      ...
      <!-- style pour les bouton laisser un avis -->
      <style>
        .btn {
  ...
  <!------------------------------------------------------------------------------------------------------------------->
<!--
  <section id="avis-defiles">
  <h3>Ce que nos visiteurs disent</h3>
  <div class="avis-scroll-container" aria-label="Défilement des avis clients">
...
<script>
  document.addEventListener("DOMContentLoaded", function () {
    ...
    function chargerAvis() {
      fetch("./avis-api.php")
        ...
        .then((avisData) => {
          avisData.forEach((avis) => {
            if (avis.message && avis.auteur) { // Vérification des données
              const avisElement = document.createElement("div");
              avisElement.className = "avis";
              avisElement.innerHTML = `
                <p>“${avis.message}”</p>
          ...
          // Dupliquer les avis si nécessaire
          if (avisContainer.scrollWidth > avisContainer.offsetWidth) {
            const avisElements = document.querySelectorAll(".avis");
            avisElements.forEach((avis) => {
              const clone = avis.cloneNode(true);
              avisContainer.appendChild(clone);
    ...
    // Fonction de défilement automatique
    let scrollAmount = 0;
    const scrollSpeed = 2; // Ajuster la vitesse
    const avisScrollContainer = document.querySelector(".avis-scroll-container");

    function scrollAvis() {
      scrollAmount = (scrollAmount + scrollSpeed) % (avisContainer.scrollWidth / 2);
      avisScrollContainer.scrollLeft = scrollAmount;
      requestAnimationFrame(scrollAvis);
    }
  ...
  <section id="avis">
    <h3>Ce que nos visiteurs disent</h3>
    <div class="avis-scroll-container"><!-- ajouter  --><!-- avis-scroll-container -->
      <div class="avis-container">
        <!-- Les avis validés seront insérés ici dynamiquement -->
      </div>
    </div>
...
.avis-scroll-container {
  overflow: hidden; /* Masquer la scroll-bar */
  white-space: nowrap;
  width: 100%;
  position: relative;
}
